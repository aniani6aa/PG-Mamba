...
def load_data(fname):
    ...
    return train_X,test_X,label_train,label_test,judge
...
def create_data_object_list(time_series_data, k=4, time_window=6,sigma=1.0):
    data_list = []
    ...
    for sample_idx in range(len(time_series_data)):
        ...
        time_edge_count = len(time_edges)
        spatial_edge_count = len(feature_edges)
    
        edge_type = torch.cat([
        torch.ones(time_edge_count, dtype=torch.long) * 1,  # edge_time=1
        torch.zeros(spatial_edge_count, dtype=torch.long)    # edge_spatial=0
        ])
        #all_edges = list(set(time_edges + feature_edges))
        all_edges = time_edges + feature_edges
        ...
        for i in range(edge_index.shape[1]):
            ...
            dist = torch.exp(-torch.tensor(dist)**2 / (2 * sigma**2))
            #print("dist",dist)
        ...
        #print(node_features.shape,edge_index.shape,edge_attr.shape,edge_type.shape)
...
class MambaGNN(nn.Module):
    def __init__(self, input_dim, hidden_dim=128,batch_size = 16,patch_num=10,gnn_dropout=0.3):
        ...
        self.gat = GATConv(hidden_dim, hidden_dim, heads = 2, dropout = gnn_dropout)
    ...
    def forward(self, data):
        ...
        #print(x[0].shape,data.edge_attr[0].shape)
        edge_attr_emb = self.edge_encoder(data.edge_attr)
        #x = torch.cat([x,data.edge_attr])
        #the reconstruction of the adjacency matrix
        
        #edge_attr add weight
        spatial_mask = (data.edge_type == 0)
        temporal_mask = (data.edge_type == 1)
        
        spatial_edge_index = data.edge_index[:, spatial_mask]
        spatial_edge_attr = edge_attr_emb[spatial_mask]
        
        temporal_edge_index = data.edge_index[:, temporal_mask]
        temporal_edge_attr = edge_attr_emb[temporal_mask]
       
        temporal_feat = self.temporal_gat(
            x, 
            temporal_edge_index, 
        ...
        spatial_feat = self.spatial_gat(
            x, 
            spatial_edge_index, 
            edge_attr=spatial_edge_attr
        )  # (num_nodes, hidden_dim)
        
        
        gate_weights = self.fusion_gate(
            torch.cat([x, temporal_feat,spatial_feat], dim=1)
        )  # (num_nodes, 3)
        
        
        fused_feat = (
            gate_weights[:, 0].unsqueeze(1) * x +
            gate_weights[:, 1].unsqueeze(1) * temporal_feat + 
            gate_weights[:, 1].unsqueeze(1) * spatial_feat 
        )  # (num_nodes, hidden_dim)
        
        x = self.feature_enhancer(fused_feat)
        
        #output, (h_n, c_n) = self.lstm(fused_feat.reshape(-1,self.patch_num,self.hidden_dim))
...
class DeepClustering(nn.Module):
    def __init__(self, encoder, n_clusters, alpha=1.0):
        ...
        self.cluster_projector = nn.Linear(encoder.hidden_dim, n_clusters)
    ...
    def forward(self, data):
        ...
        cluster_logits = self.cluster_projector(z)  # [n, num_clusters]
        cluster_probs = F.softmax(cluster_logits, dim=-1)
        
        q = cluster_probs.detach() 
        p_target = (q ** 2) / (q.sum(0) + 1e-8)  
        p_target = (p_target.t() / p_target.sum(1)).t()  
            
        cluster_loss = F.kl_div(cluster_probs.log(), p_target, reduction='batchmean')
        return z, q,recon,cluster_loss
...
def train_cluster(model, dataloader, optimizer, n_epochs=50):
    ...
    for epoch in range(n_epochs):
        ...
        for data in dataloader:
            ...
            z, q,recon,cluster_loss= model(data)
...
def count_parameters(model):
    ...
    return human_readable(total_params), human_readable(trainable_params)
...
if __name__ == '__main__':
    ...
    train_X,test_X,label_train,label_test,judge= load_data(args.dataset)
    x_init = x = torch.Tensor(train_X)
    test_X = torch.Tensor(test_X)
    
    # Load training data
    root =  os.path.join(project_root, 'code/cluster/')
    ...
    test_X= MSmodel(test_X).detach().cpu().numpy()
    train_graph_dataset = create_data_object_list(x)
    test_graph_dataset = create_data_object_list(test_X)
    aug_dataset = AugmentedDataset(train_graph_dataset)
    test_X_dataset = AugmentedDataset(test_graph_dataset)

    train_dataloader = DataLoader(aug_dataset, batch_size=args.batch_size, shuffle=False)
    test_dataloader = DataLoader(test_X_dataset, batch_size=args.batch_size, shuffle=False)
    ...
    total,train = count_parameters(cluster_model)
    print(total,train)
    import os
    ...
    if os.path.exists(model_path):
        ...
        cluster_model = train_cluster(cluster_model, train_dataloader, opt_cluster,int(args.num_epochs/3))
    ...
    with torch.no_grad():
        for data1 in test_dataloader:
            z1,q1,_,_ = cluster_model(data1[0].to(device))
            embeddings_test.append(z1.cpu())
        embeddings_test = torch.cat(embeddings_test)

   
    scaler = StandardScaler()
    embeddings_test = scaler.fit_transform(embeddings_test)
    ap = AffinityPropagation().fit(embeddings_test)
    y_pred = ap.predict(embeddings_test)
    
    
    end_time = time.time()
    duration = end_time - start_time
   
    ari = adjusted_rand_score(label_test, y_pred)
    ri = rand_score(label_test, y_pred)
    nmi = normalized_mutual_info_score(label_test, y_pred)
    
    
    
    print(f"ari {ari:.4f} ri {ri:.4f} nmi {nmi:.4f} duration {duration:.4f}")
